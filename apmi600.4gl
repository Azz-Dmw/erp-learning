...
# Modify.........: No.WEB-D40037 13/04/28 By qiull 电子采购设定相关BUG修改
# Modify.........: No.FUN-D40103 13/05/07 By fengrui ime_file添加imeacti;添加庫位有效性檢查
# Modify.........: No.MOD-D50093 13/05/14 By Vampire i600_pmc22() 傳入的是p_cmd ，而非g_pmc.pmc22 
...
# Modify.........: No.FUN-D10124 13/09/06 By yihsuan 增加【常用備註】Action
# Modify.........: No.FUN-DA0094 13/10/23 By wangrr pmc14新增選項7:客戶
...
# Modify.........: No:MOD-E60143 14/06/30 By Johnson 修正資料新增時,沒有將key值寫入azo_file中,導致aooq030查出的資料沒有key值.
# Modify.........: by nie20160215 画面档厂商分类修改为必填栏位
# Modify.........: By Hao211023 修改3个自定义字段(pmcud02/pmcud03/pmcud04)
# Modify.........: By Hao230518 资料清单添加字段
# Modify.........: By Hao230518 资料清单添加字段
...
GLOBALS "../../../tiptop/sub/4gl/s_data_center.global"   #No.FUN-820034
...
DEFINE  g_pmc_l    DYNAMIC ARRAY OF RECORD   #程式變數(Program Variables)
          ...
          pmcud03  LIKE pmc_file.pmcud03,
          pmcud04  LIKE pmc_file.pmcud04,
          pmcud05  LIKE pmc_file.pmcud05
          # Modify.........: By Hao230518
                   END RECORD
...
MAIN
   ...
   OPEN WINDOW i600_w WITH FORM "cpm/42f/apmi600" #by nie20160215 画面档厂商分类修改为必填栏位
...
#   CALL cl_set_comp_visible("pmc930,pmc930_desc,pmc58,pmc59,pmc60",g_azw.azw04='2') #FUN-870100  #TQC-B40008 mark
   CALL cl_set_comp_visible("pmc1912,reason1,pmc1913,reason2,pmc1914,pmc1915",g_azw.azw04='2') #modify by dmw20260421  隐藏字段
   ...
   CALL cl_set_comp_entry("pmcud02,pmcud03,pmcud04,pmcud05",FALSE) # Modify By Hao211023 
...
# FUN-B80093 add START
   ...
   END IF
# FUN-B80093 add END
# Modify....begin: By Paul230518 HS沒新增權限的隱藏資料清單
  IF g_dbs<>'jf' AND g_dbs<>'jh' THEN 
   SELECT zy02 FROM zx_file, zy_file 
    WHERE zx01 = g_user AND zx04 = zy01 AND zy02 = g_prog AND (zy03 LIKE '%insert,%' OR zy03 LIKE '%update,%')
   UNION 
   SELECT zy02 FROM zxw_file, zy_file 
    WHERE zxw01 = g_user AND zxw03 = '1' AND zxw04 = zy01 AND zy02 = g_prog AND (zy03 LIKE '%insert,%' OR zy03 LIKE '%update,%')
   UNION 
   SELECT zxw04 FROM zxw_file
    WHERE zxw01 = g_user AND zxw03 = '2' AND zxw04 = g_prog AND (zxw05 LIKE '%insert,%' OR zxw05 LIKE '%update,%')
   ...
   END IF
  END IF 
# Modify......end: By Paul230518 HS沒新增權限的隱藏資料清單
...
FUNCTION i600_cs()
 ...
 ELSE
   CONSTRUCT BY NAME g_wc ON                    # 螢幕上取條件
      ...
      pmc30, pmc04, pmc901,
# Modify....begin: By Paul241112 增加pmc23折讓到期日控制
#     pmc17, pmc49, pmc22 , pmc47, pmc54,  pmc911, pmc29, #CHI-A70037 add pmc29
      pmc17, pmc23, pmc49, pmc22 , pmc47, pmc54,  pmc911, pmc29, #CHI-A70037 add pmc29
# Modify......end: By Paul241112 增加pmc23折讓到期日控制
...
#     ON ACTION 重要備註
      ...
      ON ACTION com_memo
         ...
         IF cl_chk_act_auth() THEN
            IF NOT cl_null(g_pmc.pmc01) THEN
               LET l_cmd = "apmt402 '4' '",g_pmc.pmc01,"' '0'" CLIPPED
               CALL cl_cmdrun(l_cmd)
            END IF
         END IF
      #--FUN-D10124 add end--
...
#genero --no s_pmcqry
        ...
        ON ACTION confirm
           ...
           IF cl_chk_act_auth() THEN
              CALL i600_confirm()
              CALL i600_show_pic()
             #CALL i600_carry() #MOD-DC0065 add #TQC-DC0099 mark
           END IF
...
# Modify......end: By Paul241112 增加pmc23折讓到期日控制
      ...
      CASE aws_spccli_base('pmc_file',base.TypeInfo.create(g_pmc),'insert')
         WHEN 0  #無與 SPC 整合
              MESSAGE 'INSERT O.K'
              COMMIT WORK
              CALL cl_flow_notify(g_pmc.pmc01,'I')   #FUN-E50065
         WHEN 1  #呼叫 SPC 成功
      ...
      EXIT WHILE
   END WHILE
   LET g_data_keyvalue = g_pmc.pmc01       #MOD-E60143 add
END FUNCTION
...
FUNCTION i600_i(p_cmd)
   ...
   #DEFINE l_num        LIKE  type_file.num5     #No.TQC-C70088 #FUN-D60124 mark
...
#     g_pmc.pmc17, g_pmc.pmc49, g_pmc.pmc22 ,
      g_pmc.pmc17, g_pmc.pmc23, g_pmc.pmc49, g_pmc.pmc22 ,
# Modify......end: By Paul241112 增加pmc23折讓到期日控制
...
#      g_pmc.pmc930,g_pmc.pmc58,g_pmc.pmc59,g_pmc.pmc60,  #FUN-870100                    #TQC-B40008 mark
      ...
      BEFORE INPUT
          ...
          CALL cl_set_comp_entry("pmcud02,pmcud03,pmcud04,pmcud05",FALSE) 
      ...
      AFTER FIELD pmc22  		        #幣別
         IF NOT cl_null(g_pmc.pmc22) THEN
            IF cl_null(g_pmc_o.pmc22) OR cl_null(g_pmc_t.pmc22)
              ...
              #CALL i600_pmc22(g_pmc.pmc22) #MOD-D50093 mark
               CALL i600_pmc22(p_cmd)       #MOD-D50093 add
               IF NOT cl_null(g_errno) THEN
                  CALL cl_err(g_pmc.pmc22,g_errno,0)
                  LET g_pmc.pmc22 = g_pmc_o.pmc22
                  DISPLAY BY NAME g_pmc.pmc22
                  #CALL i600_pmc22(g_pmc.pmc22) #MOD-550023 #MOD-D50093 mark
                   CALL i600_pmc22(p_cmd)       #MOD-D50093 add
                  NEXT FIELD pmc22
               END IF
            END IF
...
# Modify....begin: By Paul241112 增加pmc23折讓到期日控制
      AFTER FIELD pmc23                       #付款條件
         IF g_pmc.pmc23 NOT MATCHES '[YN]' THEN
            NEXT FIELD pmc05
         ELSE
            LET g_pmc_o.pmc23 = g_pmc.pmc23
         END IF
# Modify......end: By Paul241112 增加pmc23折讓到期日控制
         ...
         END IF
{
      ...
      AFTER INPUT  #判斷必要欄位之值是否有值,若無則反白顯示,並要求重新輸入
         ...
         END IF
          ...
          # Modify By Hao211023
         -- IF (g_pmc.pmcud02 ='N' OR g_pmc.pmcud03 ='N' OR g_pmc.pmcud04 ='N') AND p_cmd = 'a' THEN 
...
DEFINE l_i             LIKE type_file.num10
    ...
    FOREACH i600_list_cur INTO l_pmc01
       IF SQLCA.sqlcode THEN
          ...
          CONTINUE FOREACH
       END IF
       SELECT pmc01,pmc03,pmc081,pmc24,pmc14,pmc02,pmy02,
              pmc30,pmc04,pmc901,pmc05,pmcacti,
              pmc1920,pmc17,pma02,pmc091,pmc52,pmc56,pmc55,nmt02,pmc10,pmc11,pmc40,
              pmcud02,pmcud03,pmcud04,pmcud05
         INTO g_pmc_l[l_i].*
         FROM pmc_file
         LEFT JOIN pma_file ON pma01=pmc17
         LEFT JOIN nmt_file ON nmt01=pmc55
...
#FUN-9A0056 add str---
	...
	ELSE
	COMMIT WORK
        CALL cl_flow_notify(g_pmc.pmc01,'U')   #FUN-E50065
	END IF
#FUN-9A0056 add end--
...
#No.FUN-A90019 -end--
      ...
      #--FUN-D10124 add start--
	DELETE FROM pmo_file WHERE pmo01 = g_pmc.pmc01 AND pmo02 = '4'
        IF SQLCA.sqlcode THEN
           CALL cl_err3("del","pmo_file",g_pmc.pmc01,"",SQLCA.sqlcode,"","",1)
           ROLLBACK WORK
           RETURN
        END IF
      #--FUN-D10124 add end--
   ...
   CLOSE i600_cl
   COMMIT WORK
   CALL cl_flow_notify(g_pmc.pmc01,'D')   #FUN-E50065
END FUNCTION
...
#  IF NOT cl_confirm('aim-301') THEN RETURN END IF    #是否確定執行確認(Y/N)? #CHI-C30107 mark
  ...
  #FUN-9A0056 add begin ----
   ...
   ELSE
      COMMIT WORK
      CALL cl_flow_notify(g_pmc.pmc01,'Y')  #FUN-E50065
   END IF
  #FUN-9A0056 add end ------
...
#   INPUT BY NAME g_wpa.wpa02,g_wpa.wpa03,       #FUN-A90021 mark
   ...
   BEFORE INPUT
         ...
         #IF g_wpa.wpa03 = 'N' THEN                                   #WEB-D40037---mark---
         IF g_wpa.wpa02 = 'N' THEN                                    #WEB-D40037---add---
      ...
      ON CHANGE wpa02                               #FUN-A90021 modify wpa03 to wpa02
         ...
         ELSE
            IF g_wpa_t.wpa02 = 'Y' and g_wpa.wpa02 = 'N' THEN             
               IF NOT(cl_null(g_wpa.wpa06) AND cl_null(g_wpa.wpa07) AND cl_null(g_wpa.wpa08)) THEN     #WEB-D40037---add---
                  ...
                  END IF
               ELSE                                                   #WEB-D40037---add---
                  CALL cl_set_comp_entry("wpa06,wpa07,wpa08",FALSE)   #WEB-D40037---add---
               END IF                                                 #WEB-D40037---add---
            END IF
         END IF
         LET g_wpa_t.wpa02 = g_wpa.wpa02                              #WEB-D40037---add---
#No.FUN-A90021  --start--           	
#      AFTER FIELD wpa03
...
#                      AND wpa03 = g_wpa_1[l_ac2].wpa03
     ...
     ON ACTION CONTROLP
       CASE
         WHEN INFIELD(wpa03)
         LET g_wpa_1_t.wpa03 = g_wpa_1[l_ac2].wpa03    #WEB-D40037---add---
         CALL q_wzx(0,0,'') RETURNING l_wpa03
         #WEB-D40037---add---str---
         IF cl_null(l_wpa03) THEN
            LET g_wpa_1[l_ac2].wpa03 = g_wpa_1_t.wpa03
         ELSE
         #WEB-D40037---add---end---
...
FUNCTION i600_sign()
   IF s_shut(0) THEN RETURN END IF                #檢查權限
	...
	IF NOT s_dc_ud_flag('5',g_pmc.pmc1920,g_plant,'u') THEN
      ...
      RETURN
	END IF
	IF g_pmc.pmcacti ='N' OR g_pmc.pmc05 ='0'  THEN    #檢查資料是否為無效
      CALL cl_err(g_pmc.pmc01,'mfg1000',0)
      RETURN
	END IF
	IF g_pmc.pmcacti = 'H' THEN
#已留置或停止交易,則不能做任何修改!
   ...
   CALL cl_set_comp_entry("pmcud02,pmcud03,pmcud04,pmcud05",TRUE)
   
   INPUT BY NAME g_pmc.pmcud02,g_pmc.pmcud03,g_pmc.pmcud04,g_pmc.pmcud05 WITHOUT DEFAULTS
   ...
   END IF
	...
	UPDATE pmc_file SET pmcud02 = g_pmc.pmcud02,pmcud03 = g_pmc.pmcud03,pmcud04 = g_pmc.pmcud04,pmcud05 = g_pmc.pmcud05,
                       pmcmodu = g_pmc.pmcmodu,pmcdate = g_pmc.pmcdate
      WHERE pmc01 = g_pmc01_t
	IF SQLCA.SQLERRD[3]=0 THEN
      CALL cl_err3("upd","pmc_file",g_pmc01_t,"",SQLCA.sqlcode,"","",1)  #No.FUN-660129
      LET g_success = 'N'
