...
# Modify.........: No.TQC-D60014 13/06/05 By yangtt 1、單身中添加規格欄位顯示
#                                                   2、單身點退出報-201錯
#                                                   3、品名pmn041改成rvv031
# Modify.........: No.FUN-D90004 13/09/03 By yangtt 在單身已開票數量前加上未開發票數量欄位(rvv87-rvv23)
# Modify.........: No.FUN-DA0083 13/10/21 By wangrr 單身增加欄位'VMI退貨否'rvv89
# Modify.........: No.MOD-DC0031 13/13/05 By yinhy 無採購單入庫幣種抓取rvu113
# Modify.........: By Hao200505 添加"请购部门"、"未含税单价"
# Modify.........: By Hao200904 添加请购单备注栏位，便于财务统计单位费用
# Modify.........: By Hao201229 添加采购人员和采购部门
# Modify.........: By dmw20260402 添加项目编号pml12字段取自采购单明细
   ...
   #FUN-D40016--mark--end
    g_body DYNAMIC ARRAY OF RECORD
           ...
           #pmn041   LIKE pmn_file.pmn041,   # P/O Item description  #TQC-D60014
            ...
            rvv38    LIKE rvv_file.rvv38,    #By Hao200505
            rvv38t   LIKE rvv_file.rvv38t,   #U/P
            #No.TQC-7B0083  --Begin
...
FUNCTION q112_b_askkey()
   #No.TQC-7B0083  --Begin
  #CONSTRUCT tm.wc2 ON rvv02,rvv04,rvv05,rvv36,rvv37,rvv31,rvv38t,rvv87,#FUN-D40016 mark
   CONSTRUCT tm.wc2 ON rvv03,rvv01,rvv02,rvv09,rvv06,rvv04,rvv05,rvv36,rvv37,
                       pmm12,pmm13,pmk13,
                       rvv31,rvv38t,rvv87,     #mod by FUN-D40016 2 120530
                       rvv23,rvv88,rvv89,rvb22,rvv25  #FUN-DA0083 add rvv89
      #FROM  s_rvb[1].rvv02,#FUN-D40016 mark
        FROM  s_rvb[1].rvv03,s_rvb[1].rvv01,s_rvb[1].rvv02,   #mod by FUN-D40016 2 120530
             ...
             s_rvb[1].pmm12,s_rvb[1].pmm13,s_rvb[1].pmk13,
             s_rvb[1].rvv31,s_rvb[1].rvv38t,
             s_rvb[1].rvv87,s_rvb[1].rvv23,
             s_rvb[1].rvv88,s_rvb[1].rvv89,s_rvb[1].rvb22,s_rvb[1].rvv25 #FUN-DA0083 add rvv89
   #No.TQC-7B0083  --End
      #No:FUN-580031 --start--     HCN
      BEFORE CONSTRUCT
         CALL cl_qbe_display_condition(lc_qbe_sn)
...
FUNCTION q112_b_fill()              #BODY FILL UP
   DEFINE l_sql     STRING  #No.FUN-690028 VARCHAR(600)

   #By Hao200904 
   LET l_sql = "select pmk13,gem02,pml06 from pml_file",
               " left join pmn_file on pml01=pmn24 and pml02=pmn25",
               " left join pmk_file on pml01=pmk01",
               " left join gem_file on pmk13=gem01",
               " where pmn01=? and pmn02=?"
   PREPARE pmk13_pre FROM l_sql
   DECLARE pmk13_cs CURSOR FOR pmk13_pre
...
#       "       OUTER ",g_dbs_tra CLIPPED,"pmm_file ",
        ...
        "  LEFT OUTER JOIN ",cl_get_target_table(source,'pml_file'),
        "          ON pml01 = pmn24 and pml02 = pmn25",
        "  LEFT OUTER JOIN ",cl_get_target_table(source,'pmk_file'),
        "          ON pml01 = pmk01",
        
        "  LEFT OUTER JOIN ",cl_get_target_table(source,'pmc_file'),#FUN-D40016 add
        "          ON rvv06 = pmc01",#FUN-D40016 add
        "  LEFT OUTER JOIN ",cl_get_target_table(source,'gen_file'),
        "          ON gen01 = pmm12",
        "  LEFT OUTER JOIN ",cl_get_target_table(source,'gem_file'),
        "          ON gem01 = pmm13",
#FUN-A50098--mod--end
       #FUN-980091-----------(E)
       #END FUN-630043
   ...
   #CALL cl_parse_qry_sql(l_sql,g_head_1.source) RETURNING l_sql #FUN-980091    #FUN-A50098
    ...
    FOREACH q112_bcs INTO g_body[g_cnt].*
        ...
        #By Hao200505 #By Hao200904
        OPEN pmk13_cs USING g_body[g_cnt].rvv36,g_body[g_cnt].rvv37
        FETCH pmk13_cs INTO g_body[g_cnt].pmk13,g_body[g_cnt].gem02,g_body[g_cnt].pml06
        CLOSE pmk13_cs
